...
```{exercise}
Use Monte Carlo to Estimate the value of $\pi$.
```
**Solution.**
```{r}
nsim <- 1000000
...
```
Now we write something is the picture will be on the right, I hope. 
```{r}
library(tidyverse)
library(kableExtra)
nsim <- 1000000
...
mutate( z = as.numeric( (x^2+y^2) < 1) )
kable(head(S, 10), digits=4) %>%  kable_styling(full_width = F)
```
