...
3.\ They are probably not correlated since the sample correlation between `x`
and `y` is close to $0$:
```{r}
(cor(x,y))
```
...
\PP[X \in [a,b]] \times \PP[ Y \in [c,d] ] = \PP[ X\in [a,b]
\text{ and } Y\in [c,d]]\text{ for all } a,b,c,d.$$ While up to the point,
...
```{exercise}
Random variables $X$ and $Y$ have the joint distribution given by the following
table:
...
variable with parameter $\ld = 4$ using $n=10$, $n=1,000$ and $1,000,000$
...
**Theorem** Let $X\_1,X\_2, \dots$ be an independent sequence of random
...
of the formula above, and, if $n$ is large enough, we hope that the this average
...
two ways. First, we can think of $Y=X^2$ as a random variable in its own and you
...
if you know how to simulated a random variable, you also know how to simulated
...
x>5
```
Therefore, the vector `y` in the solution is a vector of length $10000$ whose elements are either `TRUE` or `FALSE`; here are the first 5 rows of data frame with columns `x` and `y` from our solution:
```{r echo=FALSE}
set.seed(2123)
...
$$ \Big(x_1, \frac{1}{2} (x_1+x_2), \frac{1}{3}(x_1+x_2+x_3), \dots, \frac{1}{n} (x_1+x_2+\dots+x_n)\Big).$$ Test it to check that it really works. (*Hint:* look up the funcion `cumsum`. )
