...
3. Do `x` and `y` "look independent"? What is the mathematical definition of independence? Is there a practical way to test  independence between two simulated random variables? 
```
**Solution:**
...
2. A solution that requires less technology would start the same way, i.e., by dividing the $x-y$ plane into bins, but instead of the third dimension, it would use different colors to represent the counts. Here is an example where the bins are hexagons (and not squares; it just looks better, for some reason)
...
3. Another possibility which requires deeper statistical tools is the density contour plot:
