...
```{exercise}
Let `x` contain $10,000$ draws from $N(0,1)$, `z` another $10,000$ draws from $N(0,1)$ and let `y=x+z`. 

1. Draw a scatterplot of `x` and `y`.

2. Is there a better way to visualize the joint distribution of `x` and `y`? Would it be enough to simply plot two histograms, one of `x` and one of `y`?

3. Do `x` and `y` "look independent"? What is the mathematical definition of independence? Use the permutation test to check of independence between `x` and `y`. 
```
**Solution:**
...
2. Yes, we could use 3d  histogram, where the $x-y$ plane is divided into squares and a bar (a skyscraper) is drawn above each square. The height of each skyscraper is proportional to the number of points which fall into its base. This is the most literal analogue of a regular (2d) histogram in 2 dimensions. The one shown below can be rotated and zoomed by the reader (as long as JavaScript is enabled):
...
Displaying just the two histograms would not be enough. Here they are, side to side
```{r}
par(mfrow=c(1,2))
hist(x, freq = FALSE)
hist(y, freq = FALSE)
```
And here is a 2d histogram of another joint distribution between `x` and `y` which would give exactly the same (1D) histograms. 
```{r echo=FALSE}
library(hexbin)
df=data.frame(x=x,y=sample(y))
hexbinplot(y~x, data=df, colramp = heat.ob, asp=1)
```
3. No, they don't look independent. Vary large and very small values of `x` correspond to larger values of `y` than the intermediate values. Mathematically, we say that random variables $X$ and $Y$ are independent if
$$ \PP[X \in [a,b]] \times \PP[ Y \in [c,d] ] = \PP[ X\in [a,b] \text{ and } Y\in [c,d]].$$
To apply the permutation test, we first plot the scatterplot of `x` vs. `y` as above. Then, we replace `y` by a vector with the same components, but with randomly permuted position, and then plot a scatterplot again
```{r echo=2:12}
par(mfrow=c(2,2))
y_perm_1 = sample(y)
y_perm_2 = sample(y)
...
```
Since the upper left image looks clearly different from the other three, we conclude that `x` and `y` are likely not to be independent. 

**Comments.**

R. There is a lot of new R here, but most of it is just to make pretty plots. The only thing you need to remember is that `plot(x,y)` gives you a scatterplot of `x` and `y`. Another useful command is `par(mfrow=c(m,n))` which tells R to plot the next $mn$  plots in a $m\times n$ grid. The extra argument `xlim=c(-4,4)` sets the range of the $x$-axis; there is also `ylim`. All of there are completely cosmetic, but come in very handy when you want to compare several plots as in the case of the permutation test above. 
...
Intuitively, this means that the knowledge of one of them (say $X$) does not effect the prediction of the other
one ($Y$). In our case, if we are told that `x` is around $-2$, we would expect `y` to be around `4`, 
while if we are told that `x` is around $0$, `y` would be expected to be around $0$, too. 
