...
```{r echo=FALSE}
flip_after = function (omega, l) {
  if (l==0) {
    return(-omega)
  } else {
    T = length(omega)
    k = match(l, omega, nomatch=length(omega)+1)
    if ( k > T) {
      return(omega)
    } else {
      increments = diff(omega)
      increments[ k:(T-1)] = - increments [k:(T-1)]
      return(c(0,cumsum(increments)))
    }
  }
}

omega = as.vector(simulate_walk(1,T=12))
fomega = flip_after(omega, 3)

df = data.frame(omega, fomega) %>% pivot_long(name="x")
