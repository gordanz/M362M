...
That is it. A sample Rmd file with solutions to parts of some of the problems below can be downloaded [/testing/HW_template.html](here).

Theoretical problems (i.e., those that do not involve any computation in R) can be submitted in a seprarate document (hand written and then scanned into a pdf, for example). If you know some LaTeX, you can use an R notebook  and simply type the formulas in LaTeX notation. This way, if the assignment comes with both computational and theoretical problems, you can submit both in a single file. I am not assuming that you do know LaTeX and I am not going to require you to learn it, but if you do, use it to make your submissions nicer and easier to read (the grader will thank you!). 
...
3. Define a $2\times 2$ matrix $A=\begin{pmatrix} 1 & 2 \\ -1 & 3 \end{pmatrix}$. Use the command `matrix` and make sure to understand how it works; you can access its documentation directly from $R$ by writing `?matrix`.
